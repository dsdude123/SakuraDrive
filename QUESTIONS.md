...
answered below. **One thing is left for you**: adding the fourteen HDD pool disks as
catalog roots. That needs no drive letters and no mount points — the agent reads every
root now, and §5a explains why the architecture changed.
...
**The drvfs question is answered, and the answer is no.** WSL2 will not follow a folder
mount point into another volume:
...
That is drvfs refusing the traversal, not a permission or timing problem, and it did not
vary across the fourteen. A mount point makes a disk reachable from Windows and *not*
from the container — the opposite of what we need.

**So: drive letters.** Re-run with `-AssignDriveLetter` and the same picker gives each
selected volume the next free one, skipping A, B and anything in use. A lettered volume
appears at `/mnt/<letter>` with no traversal involved, which is the only arrangement WSL2
supports. You have 19 free and need 14; the script allocates all of them up front and
stops before touching anything if there are not enough, rather than lettering half the
pool and failing.

The folder mount points already created can stay or go — `-Remove` takes them away and
the volume is untouched either way. The agent does not care: it probes letters, mount
points and volume GUID paths, so SMART, pool membership and duplication work regardless.
Only the container's bind mounts needed the letter.

Afterwards the script prints the `/mnt/<letter>` for each disk. Put those into the 14
commented lines in `docker/docker-compose.yml`, uncomment them, and add one `poolpart`
catalog root per disk with its drive label.

**If you pulled before commit `fe91b32`, pull again.** The first version would not parse
on your server, and neither would any other file under `agent/` — the agent and its
installer included. Not a PowerShell 7 problem: Windows PowerShell 5.1 reads a `.ps1` as
the ANSI codepage unless the file has a UTF-8 BOM, so a UTF-8 em dash arrived as three
Windows-1252 characters ending in `”` — a smart closing quote, which PowerShell accepts
as a string delimiter. The string ended early and everything after it fell apart, which
is exactly the `Unexpected token 'pick'` at line 407. All 27 em dashes are gone, every
agent file is pure ASCII, and three tests plus a CI job keep it that way.

That audit also turned up two genuine 5.1 incompatibilities that would have bitten later:
`Invoke-RestMethod -SkipCertificateCheck` does not exist before PowerShell 6 (it now sets
and restores the certificate callback instead), and `Get-PhysicalDisk -DeviceNumber` is
missing from the Storage module on Server 2019 (the disks are fetched once and matched on
`DeviceId`). CI now runs PSScriptAnalyzer against a real Server 2019 / 5.1 profile over
every file the host executes.
...
752 tests green: 567 Node (161 shared, 369 server, 37 UI) and 185 Pester for the agent.
...
The Pester suite now runs here too, not just in CI: 185 tests against the real `dpcmd`,
`rxpcc status`, `rxpcc ls` and `rxpcc perf -a` output you pasted, plus the encoding and
5.1-compatibility guards described in §5a.

Not verified, because I have no access: real Windows cmdlets and smartctl against your
controllers, a live Kopia repository, and `drvfs` throughput — the first scan will tell you
that one. Whether drvfs follows a folder mount point is no longer open: it does not, and
§5a records what it does instead.
